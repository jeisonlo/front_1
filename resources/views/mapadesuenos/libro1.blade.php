...
.btn-leer-libro {
    ...
    padding: 10px 20px;
    background-color: #7a00cf; /* Color del botón */
    color: white;
...
.btn-leer-libro:hover {
    background-color: #59009A; /* Cambio de color al pasar el mouse */
    transform: scale(1.05);
}
    ...
    <main class="main">
        ...
        <div class="book-actions">
            <p><strong></strong><a id="link" href="#" class="btn-leer-libro" target="_blank">Leer libro  </a></p>  </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let userRating = 0; // Almacena la calificación del usuario actual

    // Intentar obtener la calificación del usuario de localStorage (si existe)
    const storageKey = `libro_${LIBRO_ID}_userRating`;
    ...
    function updateStarDisplay(rating) {
        clearStars();
        estrellas.forEach(estrella => {
            if (parseFloat(estrella.dataset.value) <= rating) {
    ...
    // Función para cargar solo el PROMEDIO, sin modificar la visualización de estrellas
    function loadAverageRating() {
        fetch(`${RATING_API_URL}?libro_id=${LIBRO_ID}`)
            ...
            .then(data => {
                ...
                if (ratings.length > 0) {
                    // Calcular y mostrar el promedio SOLO en el texto
    ...
    async function submitRating(rating) {
        try {
            ...
            // Guardar la calificación del usuario localmente
            userRating = rating;
            localStorage.setItem(storageKey, rating.toString());
            
            // Actualizar la visualización de estrellas con la calificación del usuario
            updateStarDisplay(userRating);
            
            // Cargar SOLO el promedio numérico sin cambiar las estrellas
            loadAverageRating();
    ...
    if (estrellas.length > 0) {
        estrellas.forEach(estrella => {
            ...
            estrella.addEventListener('mouseout', function() {
                ...
                // Al quitar el mouse, volver a mostrar la calificación del usuario actual
                if (userRating > 0) {
                    updateStarDisplay(userRating);
                }
            });
    ...
    // Cargar solo el promedio numérico al inicio
    loadAverageRating();
    
    // Si el usuario ya tiene una calificación guardada, mostrarla
    if (userRating > 0) {
        updateStarDisplay(userRating);
    }
